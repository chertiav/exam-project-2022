...
const validationSchems = {
	LoginSchem: yup.object().shape({
		...
		password: yup.string().test('test-password', 'min 6 symbols', (value) => (
			value && value.trim().length >= 6)).required('required'),
	}),
	RegistrationSchem: yup.object().shape({
		...
		firstName: yup.string().test('test-firstName', 'required', (value) => (
			value && value.trim().length >= 1)).required('First Name is required'),
		lastName: yup.string().test('test-lastName', 'required', (value) => (
	...
	ContestSchem: yup.object({
		...
		title: yup.string().test('test-title', 'required', (value) => (
			value && value.trim().length >= 1)).required('title of contest required'),
		industry: yup.string().required('industry required'),
		focusOfWork: yup.string().test('test-focusOfWork', 'required', (value) => (
			value && value.trim().length >= 1)).required('focus of work required'),
		targetCustomer: yup.string().test('test-targetCustomer', 'required', (value) => (
			value && value.trim().length >= 1)).required('target customers required'),
	...
	TextOfferSchema: yup.object().shape({
		offerData: yup.string().test('test-offerData', 'required', (value) => (
			value && value.trim().length >= 1)).required('suggestion is required'),
	}),
	PaymentSchema: yup.object().shape({
		number: yup.number().test('test-cardNumber', 'Credit Card number is invalid', (value) =>
			valid.number(value).isValid).required('required'),
		name: yup.string().min(1, 'required atleast one symbol').required('required'),
		cvc: yup.string().test('test-cvc', 'cvc is invalid', (value) =>
			valid.cvv(value).isValid).required('required'),
		expiry: yup.string().test('test-expiry', 'expiry is invalid', (value) =>
			valid.expirationDate(value).isValid).required('required'),
	}),
	CashoutSchema: yup.object().shape({
		sum: yup.number().min(5, 'min sum is 5$').required('required'),
		number: yup.string().test('test-cardNumber', 'Credit Card number is invalid', (value) =>
			valid.number(value).isValid).required('required'),
	...
	MessageSchema: yup.object({
		message: yup.string().test('test-message', 'required', (value) =>
			value && value.trim().length >= 1).required('required'),
	}),
